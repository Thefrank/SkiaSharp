...
string VARIANT = string.IsNullOrEmpty(BUILD_VARIANT) ? "freebsd" : BUILD_VARIANT?.Trim();
...
Task("libSkiaSharp")
    ...
    .WithCriteria(IsRunningOnFreeBSD())
    .Does(() =>
{
    foreach (var arch in BUILD_ARCH) {
        ...
        var so = SKIA_PATH.CombineWithFilePath($"out/{VARIANT}/{arch}/libSkiaSharp.so");
    ...
    .Does(() =>
{
    foreach (var arch in BUILD_ARCH) {
        ...
        var outDir = OUTPUT_PATH.Combine($"{VARIANT}/{arch}");
        EnsureDirectoryExists(outDir);
        var so = SKIA_PATH.CombineWithFilePath($"out/{VARIANT}/{arch}/libHarfBuzzSharp.so");
        CopyFileToDirectory(so, outDir);
